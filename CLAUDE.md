...
- **`internal/protocol`** — the wire contract shared by server and both clients: `ClientMessage` (`join`, `send`, `list_rooms`, `list_users`, `history`; `join` accepts a `Quiet` flag to suppress `user_joined`/`user_left` broadcasts) and `ServerMessage` (`ack`, `error`, `message`, `rooms`, `users`, `user_joined`, `user_left`, `history`). Changing message shapes here affects `sshserver`, `tui`, and `historyclient` simultaneously.
