...
WHERE room_id = ? AND (? = '' OR sender = ?)
ORDER BY created_at
LIMIT ? OFFSET ?`

func (d *DB) ListMessages(roomID int64, sender string, page, pageSize uint) ([]Message, error) {
	var messages []Message
	err := d.Select(&messages, listMessagesQuery, roomID, sender, sender, pageSize, (page-1)*pageSize)
	return messages, err
}
